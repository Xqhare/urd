use eframe::egui::{
    Align, CentralPanel, Color32, ComboBox, Context, Grid, ProgressBar, Sides, Slider, TextEdit,
    TopBottomPanel, Ui,
};
use horae::TimeZone;
        ...
        CentralPanel::default().show(ctx, |ui: &mut Ui| {
            ...
            ui.add_sized([ui.available_width() / 1.33, 150.0], |ui: &mut Ui| {
                ui.horizontal(|ui: &mut Ui| {
                    ui.add_space(ui.available_width() / 3.0);
                    ui.vertical_centered(|ui: &mut Ui| {
                        ui.group(|ui: &mut Ui| match self.state_store.wizard_setup_step {
    ...
    fn setup_step_1(&mut self, ui: &mut Ui) {
        ...
        ui.group(|ui: &mut Ui| {
            Sides::new().show(
                ...
                |ui: &mut Ui| {
                    ui.add(
                        TextEdit::singleline(&mut self.settings.password.new_password_input[0])
            ...
            Sides::new().show(
                ...
                |ui: &mut Ui| {
                    ui.add(
                        TextEdit::singleline(&mut self.settings.password.new_password_input[1])
                            .horizontal_align(Align::Center)
                            .password(true),
                    );
                },
            ...
            if ui.button("Set password").clicked() {
                    ...
                    == self.settings.password.new_password_input[1]
                {
                    if pw_set {
                        if self.settings.password.password == self.settings.password.password_input
                        {
                            ...
                            self.error = Error::new(
                                ...
                                "Setting new password failed.".to_string(),
                            );
                        }
                    } else {
                        self.settings.password.password =
                    ...
                    self.error = Error::new(
                        "New password entries do not match".to_string(),
                        "Setting new password failed.".to_string(),
                    );
                }
        ...
        for entry in self.render.entities.aspirations.iter_mut() {
            ui.group(|ui: &mut Ui| {
                ...
                Sides::new().show(
                    ...
                    |ui: &mut Ui| {
                        ui.add(
                            TextEdit::singleline(&mut entry.edit_theme)
                                .horizontal_align(Align::Center)
                                .hint_text("Progress"),
                        )
                        .on_hover_text("Enter the theme of the year - One word only.");
                    },
                );
                Sides::new().show(
                    ui,
                    ...
                    |ui: &mut Ui| {
                        ui.add(
                            TextEdit::singleline(&mut entry.edit_pledge)
                                .horizontal_align(Align::Center)
                                .hint_text("Walk more"),
                        )
                        .on_hover_text("Enter the pledge of the year - One sentence max.");
                    },
                );
                    ...
                    for resolution in entry.edit_resolutions.iter_mut() {
                        ui.add(
                            TextEdit::singleline(resolution)
                                .horizontal_align(Align::Center)
                                .hint_text("Go to the gym"),
                        )
                        .on_hover_text("Enter one resolution per line.");
                    }
                });
    ...
    fn setup_top_panel(&mut self, ctx: &Context) {
        TopBottomPanel::top("setup_top_panel").show(ctx, |ui: &mut Ui| {
            ui.vertical_centered_justified(|ui: &mut Ui| {
                ...
                ui.add(
                    ProgressBar::new(self.state_store.setup_wizard_progress)
                        .fill(Color32::GREEN)
                        .desired_height(5.0),
                );
            });
