...
impl UrdState {
    pub fn settings_viewport_startup(&mut self, ctx: &Context) {
        if self.settings.show_settings_viewport.load(std::sync::atomic::Ordering::Relaxed) {
            ...
            ctx.show_viewport_deferred(ViewportId::from_hash_of("settings_viewport"), ViewportBuilder::default().with_title("Settings").with_inner_size([200.0, 400.0]), move |ctx, class| {
