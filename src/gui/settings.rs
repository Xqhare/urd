...
use crate::settings::{self, Settings, MAX_FONT_SIZE, MAX_SIDE_PANEL_WIDTH, MAX_WINDOW_SIZE, MIN_FONT_SIZE};
            ...
            ScrollArea::vertical().show(ui, |ui: &mut Ui| {
                ui.group(|ui: &mut Ui| {
                    ui.vertical_centered_justified(|ui: &mut Ui| {
                                    ...
                                    ui.add_sized(ui.available_size(), |ui: &mut Ui| {
                                        ...
                                        if but.clicked() {
                                            ...
                                            if save.is_err() {
                                                ...
                                                self.error_message = Some(save.unwrap_err().to_string());
                                            }
                                            self.settings.overwrite_window_size = false;
                                ...
                                Grid::new("window_settings").num_columns(2).show(ui, |ui: &mut Ui| {
                                    ...
                                    if self.settings.overwrite_window_size {
                                        ...
                                        if overwrite_width.is_err() || overwrite_height.is_err() {
                                            self.show_error = true;
                                            self.error_message = Some("Invalid window size - Not a number".to_string());
                                            return;
                                        } else {
                                    ...
                                    if self.settings.overwrite_side_panel_width {
                                        ...
                                        if overwrite_panel_width.is_err() {
                                            self.show_error = true;
                                            self.error_message = Some("Invalid side panel size - Not a number".to_string());
                                            return;
                                        } else {
                        ...
                        ui.add(|ui: &mut Ui| {
                            ui.group(|ui: &mut Ui| {
                                let pw_set = if self.settings.password == "" { false } else { true };
                                ui.label("Security");
                                Grid::new("pw").num_columns(2).show(ui, |ui: &mut Ui| {
                                    ui.label("Old password: ");
                                    ui.add_enabled(pw_set, |ui: &mut Ui| {
                                        ui.add(TextEdit::singleline(&mut self.settings.password_input).horizontal_align(Align::Center).password(true))
                                    });
                                    ui.end_row();

                                    ui.label("New password: ");
                                    ui.add(TextEdit::singleline(&mut self.settings.new_password_input[0]).horizontal_align(Align::Center).password(true));
                                    ui.end_row();

                                    ui.label("Repeat new password: ");
                                    ui.add(TextEdit::singleline(&mut self.settings.new_password_input[1]).horizontal_align(Align::Center).password(true));
                                });
                                if ui.button("Set new password").clicked() {
                                    let mut set_pw_is_okay = false;
                                    if self.settings.new_password_input[0] == self.settings.new_password_input[1] {
                                        if pw_set {
                                            if self.settings.password == self.settings.password_input {
                                                self.settings.password = self.settings.new_password_input[0].to_string();
                                                set_pw_is_okay = true;
                                            } else {
                                                self.show_error = true;
                                                self.error_message = Some("Incorrect old password".to_string());
                                            }
                                        } else {
                                            self.settings.password = self.settings.new_password_input[0].to_string();
                                            set_pw_is_okay = true;
                                        ...
                                        self.show_error = true;
                                        self.error_message = Some("New password entries do not match".to_string());
                                    }

                                    if set_pw_is_okay {
                                        self.settings.password_input = String::new();
                                        self.settings.new_password_input[0] = String::new();
                                        self.settings.new_password_input[1] = String::new();
                        ...
                        ui.add(|ui: &mut Ui| {
                            ui.group(|ui: &mut Ui| {
                                ...
                                Grid::new("date_settings").num_columns(2).show(ui, |ui: &mut Ui| {
                                    ...
                                    ui.add_sized(ui.available_size(), |ui: &mut Ui| {
                                        ComboBox::from_label("").selected_text(self.settings.timezone.timezone.unwrap().to_string()).show_ui(ui, |ui: &mut Ui| {
                                            for tz in self.settings.timezone.all_timezones_str.iter() {
                                                if ui.selectable_value(&mut self.settings.timezone.timezone, Some(TimeZone::from(tz.clone())), tz.to_string()).clicked() {
                                                    let save = self.settings.save();
                                                    if save.is_err() {
                                                        self.show_error = true;
                                                        self.error_message = Some(save.unwrap_err().to_string());
